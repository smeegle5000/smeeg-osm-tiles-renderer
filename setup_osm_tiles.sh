...
sudo dnf install -y \
    ...
    osm2pgsql mapnik mapnik-utils python3-mapnik \
    gdal git nodejs npm \
    ...
    unifont-fonts python3-pygame

echo "=== init postgres (fedora needs manual initdb) ==="
...
echo "=== create db user matching current user ==="
sudo -u postgres createuser -s "$USER" 2>/dev/null || echo "user exists, skip"

echo "=== fresh gis db ==="
dropdb gis 2>/dev/null || true
createdb gis
...
echo "=== resolve pbf path ==="
PBF_FILE="${1:-}"
if [ -z "$PBF_FILE" ]; then
    PBF_FILE=$(find ~/Downloads ~/Documents . -maxdepth 2 -iname "*.osm.pbf" 2>/dev/null | head -n1)
fi

if [ -z "$PBF_FILE" ] || [ ! -f "$PBF_FILE" ]; then
    echo "no pbf found. rerun: ./setup_osm_tiles.sh /path/to/file.osm.pbf"
    exit 1
fi

PBF_FILE=$(realpath "$PBF_FILE")
echo "using pbf: $PBF_FILE"

echo "=== carto style repo ==="
if [ ! -d openstreetmap-carto ]; then
...
echo "=== external data (shapefiles) ==="
./scripts/get-external-data.py

echo "=== pbf import ==="
osm2pgsql -d gis --create --slim -G --output=flex --style=openstreetmap-carto-flex.lua \
    -C 2048 --number-processes "$(nproc)" "$PBF_FILE"

echo "=== functions + common values ==="
psql -d gis -f functions.sql
python3 scripts/get-common-values.py > common-values.sql
psql -d gis -f common-values.sql

echo "=== mapnik style ==="
carto project.mml > mapnik.xml

echo "=== writing tiles.py ==="
cat > tiles.py << 'PYEOF'
...
echo "python3 tiles.py /path/to/osm.pbf"
